...
void ReadFile::writeMap()
{
	...
	LENGTH ReadFile::singleSource(int source, int destination)
	{
		vector<int> vMarkPath(m_nVertex,0);

		MinHeap2 minheap;
		minheap.initial(m_nVertex);
		KEY_TYPE key(0,0,0);
		minheap.Push(key, source);

		vector<int> vPrevious(m_nVertex);
		vPrevious[source] = -1;
		int result = 0;
		while(minheap.count)
		{
			...
			//printf("(%d, %d)", vtop, currentKey.key1);
			...
			for (set<int>::iterator jj = vertexList[vtop].edge_list.begin(); jj != vertexList[vtop].edge_list.end(); ++jj)
			{
				int eid = *jj;
				int vneighbor = edgeList.edges[eid].adjVertex(vtop);

				if(vMarkPath[vneighbor] == -1)
					continue;

				int pos = minheap.index[vneighbor];

				KEY_TYPE temp(currentKey.key1 + edgeList.edges[eid].length,0,0);
					if(pos == -1)
					{
						minheap.Push(temp, vneighbor);
						vPrevious[vneighbor] = vtop;
					}
					else if (pos == minheap.NO)
						continue;
					else
					{
						assert(minheap.minheap[pos].second == vneighbor);
						if(minheap.minheap[pos].first > temp)
						{
							minheap.DecreaseKey(pos, temp);
							vPrevious[vneighbor] = vtop;
						}
		...
		cout<<endl;
		int middle = vPrevious[destination];
		while(middle != -1)
		{
			cout<<middle<<'\t';
			middle = vPrevious[middle];
		}
		cout<<endl;
		return result;
	}