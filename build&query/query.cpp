...
int main(int argc, char* argv[])
{
	...
	freqNode = &(vtree[vGoodBranch[0]]);
	freqSize = freqNode->pos0Size;
	freqPos = freqNode->pos0;



//readQuery(queryName, 1);
//return 0;
	...
	if(prune)
	{
		for (iii = 0; iii < queryCnt; ++iii)
		{
			//for(int jj=0;jj<2;++jj)
				dSum+=getDist(vec1[iii], vec2[iii]);
		}
	}
	else
	{
		...
		{
			//for(int jj=0;jj<2;++jj)
			dSum+=distanceQuery(vec1[iii], vec2[iii]);
		}
	}
	cout << tm.getTime() / queryCnt << '\t';
	cout << avgw*1.0 / queryCnt << '\t'<<g_mw<<endl;
	cout << dSum/queryCnt<<endl;
