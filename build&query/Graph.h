...
static double s_outputThredhold;
	...
	void printMe(vector<Node> & TG, int vid, FILE * fout, map<int, double> & mBidBetter)
	{
		fwrite(&vid, sizeof(int), 1, fout);

		//for each point, we need to write down the pos table
		printmPos(fout, mBidBetter);
...
public:
	...
	void removeMe(vector<VertexInH> & vh, EdgeList & eh, TreeNode & tn, bool bLeafNode)
	{
		deleted = true;
		//tn.uniqueVertex = id;
		tn.clear();
